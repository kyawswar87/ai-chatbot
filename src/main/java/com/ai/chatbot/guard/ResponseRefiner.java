 ...
 *
 * <p>Two models, two jobs:
 * <ul>
 *   <li><b>Judge</b> — a separate {@code judgeModel} ({@code llama3.2}) so the answer is
 *       graded by a model other than the one that wrote it (no self-grading bias).</li>
 *   <li><b>Refiner</b> — the <em>generator</em> model ({@code spring.ai.ollama.chat.model},
 *       i.e. {@code mistral}), since the rewrite is user-facing answer content and should
 *       match the normal RAG path's voice and quality. This is also the classic Self-Refine
 *       setup: the same model generates and then refines its own output.</li>
 * </ul>
 *
 * <p>The refinement re-generation runs through a <em>dedicated</em> {@link ChatClient} built
 * here, <b>not</b> by re-entering the advisor chain: Spring AI's
...
public class ResponseRefiner {
	...
	public ResponseRefiner(ChatModel chatModel, GuardrailProperties properties) {
		...
		// Judge: separate model (llama3.2) so it doesn't grade its own output.
		this.relevancyEvaluator = RelevancyEvaluator.builder()
				...
				.build();
		// Refiner: no model override, so it reuses the autoconfigured generator
		// (spring.ai.ollama.chat.model = mistral) — the rewrite is user-facing answer
		// content and should match the normal RAG path's model.
		this.refinerClient = ChatClient.builder(chatModel).build();
	}
